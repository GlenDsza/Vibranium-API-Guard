...
const Chatbot = () => {
  ...
  const sendMessage = async (newMessage: { role: string; message: string }) => {
    ...
    setLoading(true);
    try {
      // const res = await axios.post(`${import.meta.env.VITE_LLM_URL}/qa`, {
      //   prompt: newMessage.message,
      // });
      // await Promise.resolve(
      //   setChat((prev) => [...prev, { role: "ai", message: res.data.response }])
      // );
      await Promise.resolve(
        setChat((prev) => [
          ...prev,
          {
            role: "ai",
            message:
              "There are 4 API's which could be affected by Broken Object Level Authentication (BOLA)",
          },
        ])
      );
    } catch (error) {
